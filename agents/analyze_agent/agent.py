...
analyze_agent = LlmAgent(
    ...
    instruction="""You are an expert in VoIP technology, proficient in HTTP, WebRTC, SIP, SDP RTP, TCP, UDP and other related protocols. Structure the response in bullet points and output should be in md format.
        ...
        Context: You are analyzing logs from a WebRTC Calling flow, which involves talking to different endpoints using protocols like HTTP, SIP, WebRTC, SDP, RTP, TLS. 
        Use `serviceIndicator` from logs to classify the session:
        - `calling`, `guestCalling` → WebRTC Calling Flow
        - `contactCenter` → Contact Center Flow
        ...
        /n
        → **[Timestamp]**: METHOD request from source [SOURCE_IP] to destination [DEST_IP]
