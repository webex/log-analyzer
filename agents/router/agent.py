...
class QueryAnalyzerAgent(BaseAgent):
    ...
    def _parse_llm_intent(self, raw: str, ctx: InvocationContext) -> dict | None:
        try:
            ...
            if intent == "re_search":
                ...
                if not stored:
                    logger.info("[router] re_search but no previous search — treating as chat")
                    return None
    ...
    async def _run_async_impl(
        ...
        logger.info(f"[router] oauth_token from session: {'yes (' + str(len(oauth_token)) + ' chars)' if oauth_token else 'EMPTY'}")
        if oauth_token:
            set_oauth_token(oauth_token)
            logger.info(f"[router] Token set in contextvar for this request (per-user isolated)")
        else:
            yield Event(
                author=self.name,
        ...
        if from_frontend:
            logger.info("[router] Fast path — structured JSON")
        elif user_message:
            logger.info("[router] Slow path — running intent_parser")
            intent_text = ""
            ...
            if search_params:
                logger.info("[router] Intent parser extracted search: %s", search_params)
            else:
                logger.info("[router] Intent parser classified as chat")
        ...
        if search_params is not None:
            if search_params.get("type") == "upload_only":
                ...
                logger.info("[router] Same JSON params from frontend — skipping pipeline")
            else:
                needs_pipeline = True
        elif not has_results:
            logger.info("[router] No results and not a search — chat will handle")
        ...
        if needs_pipeline:
            ...
            if upload_only:
                ...
                if uploaded:
                    ...
                    logger.info("[router] SDK logs uploaded alongside search")
                ctx.session.state["last_search_params"] = json.dumps(
                    search_params, sort_keys=True
                )
                logger.info("[router] Running pipeline")
                async for event in pipeline.run_async(ctx):
                    yield event
        else:
            logger.info("[router] Skipping pipeline — passing to chat_agent")
            return
            yield
